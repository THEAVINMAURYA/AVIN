...
const CalendarPage: React.FC<CalendarPageProps> = ({ data, search, setSearch, dateRange, setDateRange }) => {
  ...
  // Sync calendar view with Maya's date request
  useEffect(() => {
    if (dateRange.start) {
      const targetDate = new Date(dateRange.start);
      if (!isNaN(targetDate.getTime())) {
        // If the date is valid, move the calendar to that month
        setCurr(new Date(targetDate.getFullYear(), targetDate.getMonth(), 1));
      }
    }
  }, [dateRange.start]);
  ...
  const getBalanceOnDate = (dateStr: string) => {
    ...
    const totalChange = data.transactions
      ...
      .reduce((sum, t) => (t.type === TransactionType.INCOME || t.type === TransactionType.SALE) ? sum + t.amount : sum - t.amount, 0);
  ...
  const getDayMetrics = (day: number) => {
    ...
    const inc = items.filter(i => (i.type === TransactionType.INCOME || i.type === TransactionType.SALE)).reduce((s, i) => s + i.amount, 0);
    const exp = items.filter(i => (i.type === TransactionType.EXPENSE || i.type === TransactionType.PURCHASE)).reduce((s, i) => s + i.amount, 0);
    ...
    <div className="space-y-8 animate-in">
      <header className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-6 bg-white p-2 rounded-[2rem] shadow-sm border border-slate-100">
           <button onClick={() => setCurr(new Date(curr.getFullYear(), curr.getMonth() - 1, 1))} className="w-12 h-12 flex items-center justify-center bg-slate-50 text-slate-600 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all">
      ...
      <div className="grid grid-cols-7 gap-4">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          const day = i + 1;
          const { inc, exp, eodBalance, dStr } = getDayMetrics(day);
          const isToday = new Date().toDateString() === new Date(curr.getFullYear(), curr.getMonth(), day).toDateString();
          const isFiltered = dateRange.start === dStr;
          
          return (
            <div 
              key={day} 
              onClick={() => handleDayClick(day)}
              className={`group bg-white min-h-[140px] p-4 rounded-[2rem] border transition-all cursor-pointer flex flex-col justify-between hover:shadow-xl hover:scale-[1.02] hover:border-indigo-400 ${isToday ? 'border-indigo-600 ring-2 ring-indigo-50 shadow-lg shadow-indigo-100' : isFiltered ? 'border-amber-400 ring-2 ring-amber-50 shadow-lg' : 'border-slate-100 shadow-sm'}`}
            >
             ...
             <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                ...
                <div className="flex gap-4">
                  <span className="text-emerald-500 font-black text-lg">
                    +₹{selectedDayData?.items.filter(i => (i.type === TransactionType.INCOME || i.type === TransactionType.SALE)).reduce((s, i) => s + i.amount, 0).toLocaleString()}
                  </span>
                  <span className="text-rose-500 font-black text-lg">
                    -₹{selectedDayData?.items.filter(i => (i.type === TransactionType.EXPENSE || i.type === TransactionType.PURCHASE)).reduce((s, i) => s + i.amount, 0).toLocaleString()}
                  </span>
                </div>
             </div>
            ...
            {selectedDayData?.items.map(t => (
              <div key={t.id} className="flex justify-between items-center p-5 bg-white border border-slate-100 rounded-3xl hover:border-indigo-200 transition-colors">
                ...
                <div className="text-right">
                  <p className={`font-black ${(t.type === TransactionType.INCOME || t.type === TransactionType.SALE) ? 'text-emerald-500' : 'text-rose-500'}`}>
                    {(t.type === TransactionType.INCOME || t.type === TransactionType.SALE) ? '+' : '-'}₹{t.amount.toLocaleString()}
                  </p>
